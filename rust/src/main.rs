...
fn main() -> Result<()> {
    ...
    eval_list(&Rc::new(default_env()), to_vec(Rc::new(v)))?;
    Ok(())
}
struct Parser {
    tokens: Vec<String>,
...
impl Env {
    ...
    fn with_ops(&self, s: &str, f: fn(Vec<Rc<Val>>) -> Result<Rc<Val>>) -> &Self {
        let g = move |e: &Rc<Env>, v| {
    ...
    fn with_func(&self, s: &str, f: fn(&Rc<Env>, Rc<Val>) -> Result<Rc<Val>>) -> &Self {
    ...
    fn new_frame(self: Rc<Env>) -> Rc<Env> {
        Rc::new(Env {
            m: HashMap::new().into(),
            next: Some(self),
        })
...
fn default_env() -> Env {
    ...
    res.with_fold("+", |x, y| Ok(Int(x.int()? + y.int()?).into()))
        ...
        .with_func("define", |e, v| {
            match v.pair()?.0.as_ref() {
                Symbol(name) => e.ensure(name, eval(&e, v.pair()?.1.pair()?.0)?),
                Pair(name, args) => e.ensure(
                    ...
                    eval(
                        ...
                        Rc::new(new_pair(
                            ...
                            &new_pair(&args.borrow(), &v.pair()?.1).into(),
                        )),
                    )?,
                ),
        ...
        .with_func("lambda", |e, v| {
            ...
            Ok(Rc::new(Func(Box::new(move |e2, mut v2| {
                let (e, mut args) = (e.clone().new_frame(), v.pair()?.0);
        ...
        .with_func("let", |e, v| {
            let (e2, mut kvs) = (e.clone().new_frame(), v.pair()?.0);
        ...
        .with_func("let*", |e, v| {
            ...
            while let Ok((ref kv, n_kvs)) = kvs.pair() {
                let v = eval(&e, kv.pair()?.1.pair()?.0)?;
                e = e.new_frame();
                e.ensure(kv.pair()?.0.symbol()?, v);
                kvs = n_kvs;
            }
            eval_list(&e, to_vec(v.pair()?.1))
        })
        .with_func("letrec", |e, v| {
            let (e, mut kvs) = (e.clone().new_frame(), v.pair()?.0);
            while let Ok((ref kv, n_kvs)) = kvs.pair() {
        ...
        .with_func("cond", |e, mut v| loop {
            let f = v.pair()?.0;
            if eval(&e, f.pair()?.0)?.bool() {
                break eval(e, f.pair()?.1.pair()?.0);
            }
            v = v.pair()?.1;
        })
        .with_func("set!", |e, v| {
